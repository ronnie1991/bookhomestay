    ...
    <div id="page" class="theia-exception">
        ...
        <main>
                    ...
                    @if (count($getproperty) > 0)
                        ...
                        <div class="swiper-slide">
                            <div class="explor-media">
                                ...
                                <div class="media">
                                    <img class="d-flex align-self-center mr-3" src="{{ asset('placebanner')}}/{{ $mustnot->banner }}" alt="No image">
                                    <div class="media-body">
                                        <h5 class="mt-0">{{ $mustnot->place }}</h5>
                                    </div>
                                </div>
                                </a>
                            </div>
            ...
            <div class="location-sec recommended-sec" style="padding-top:0px !important;">
                    <div class="container">
                        @foreach ($place as $cl=>$mustnot)
                         @php
                        $getproperty = DB::table('properties')->where('placeId',$mustnot->id)->limit(1)->get();
                        @endphp
                        @if (count($getproperty) > 0)
                            <div  id="{{$cl}}"></div>
                            <h1>Homestays in {{ $mustnot->place }}</h1>
                            <div class="row">
                                @php
                                               ...
                                               @foreach ($getproperty as $showpro)
                                                <div class="swiper-slide">
                                                    <div class="team-block">
                                                        <div class="box_grid">
                                                        <figure>
                                                         <a href="{{ url('/searchDetails/'.$showpro->id.'') }}" target="_blank">
                                                            <img src="{{ asset('propertypic')}}/{{ $showpro->image }}" alt="" />
                                                            </a>
                                                        <small>{{ $showpro->hotel_name }}</small>
                                                     </figure>
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                <div class="col-sm-12">
                                 <a class="show-all" href="{{ url('/explore_all_homestays/'.$mustnot->id)}}">Show all   <i class="la la-angle-right"></i></a>
                               </div> 
                            </div>
                            <br>
                            <hr>
